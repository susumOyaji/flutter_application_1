...
//SocketException (SocketException: Failed host lookup: 'finance.yahoo.co.jp' (OS Error: No address associated with hostname, errno = 7))
//Pixel_3a_API_30_x86(android-x86 emulater)
...
//MapからListへの変換,適当なMapを用意これをList<Person>に変換
void pa() {
...
//3. Iterable map()を使う
  final list1 = persons.entries.map((e) => Person(e.key, e.value)).toList();
  print(list1);
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<String> stdcodeItems = ['998407.O', '998407.O']; //Nikkei And Dw
  ...
  Future get_nikkeihtmls() async {
    ...
    RegExp regprice = RegExp(r'"stoksPrice">.*?<');
    RegExp regchangePrice = RegExp(r'yjMSt">.*?<'); //RegExp(r"(\w+)");

    RegExp regchangePriceRate = RegExp(r'（.+?）');

    List<String> price =
        regprice.allMatches(json).map((match) => match.group(0)).toList();
    nikkei.add(price[0].replaceAll(RegExp("[^-+0-9,.]"), ""));

    List<String> changePrice =
        regchangePrice.allMatches(json).map((match) => match.group(0)).toList();
    List splt = changePrice[0].split("（");
    nikkei.add(splt[0].replaceAll(RegExp("[^-+0-9,.]"), ""));

    List<String> changePriceRate = regchangePriceRate
        .allMatches(changePrice[0])
        .map((match) => match.group(0))
        .toList();
    nikkei.add(changePriceRate[0].replaceAll(RegExp("[^-+0-9,.%]"), ""));

    return nikkei;
  }

  void fetch_dow() async {
    dow_responce = await get_dowhtmls();

    print('Dow: ' + dow_responce.toString());
  }

  void fetch_std() async {
    var responce = await get_nikkeihtmls();

    print('Nikkei225: ' + responce.toString());
  }

  Future fetch(List<String> items) async {
    List<String> org_price = [];
    //var re = pa();
    //6758,6976
    ...
    for (String item in responce) {
      final response =
          await http.get('https://finance.yahoo.co.jp/quote/' + item); //^DJI
      String json = response.body;
      ...
      RegExp regprice =
          RegExp(r'<span class="_3rXWJKZF">.*?</span>'); //RegExp(r"(\w+)");
      List<String> price =
          regprice.allMatches(json).map((match) => match.group(0)).toList();

      List<String> tmpprice = [];
      for (int i = 0; i < price.length; i++) {
        tmpprice.add(price[i].replaceAll(RegExp('"_3rXWJKZF"'), ''));
        org_price.add(tmpprice[i].replaceAll(RegExp('[^-+0-9,.]'), ''));
      }
      ...
      */
      ...
      String value;
      value = org_price[0]; //現在値
      //regExp.stringMatch(json).toString(); //現在値
      ...
      String changevalue;
      tmpprice.add(price[1].replaceAll(RegExp("_3rXWJKZF"), ""));
      org_price.add(tmpprice[1].replaceAll(RegExp("[^0-9,.]"), ""));

      changevalue = regExp.stringMatch(changedata).toString();
      //changerate = changerate.replaceAll('"', "");
      //changePriceRate.add(changerate); //前日比%

      debugPrint("Changevalue : " + org_price[1]);
      debugPrint("Changerate : " + org_price[2] + '%');
      ...
      String changerate =
          org_price[2] + '%'; //regExp.stringMatch(changedata).toString();
      //changerate = changerate.replaceAll('"', "");
                                ...
                                children: <Widget>[
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      MaketStandard(
                                        dow_Value: dow_responce, //presentvalue,
